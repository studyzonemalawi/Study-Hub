...
export const Admin: React.FC<AdminProps> = ({ user, onNavigate }) => {
  const [activeAdminTab, setActiveAdminTab] = useState<'content' | 'local-upload' | 'users' | 'announcements' | 'exams'>('content');
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.type !== 'application/pdf') {
        setError("Only PDF files are supported for local upload.");
        setLocalFile(null);
        return;
      }
      setLocalFile(file);
  ...
  const handlePostAnnouncement = async (e: React.FormEvent) => {
    ...
    setSuccessMsg('Broadcast sent to all students');
    setTimeout(() => setSuccessMsg(null), 3000);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
        <div className="flex bg-white dark:bg-slate-800 p-1.5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-x-auto no-scrollbar">
          {(['content', 'local-upload', 'exams', 'users', 'announcements'] as const).map((tab) => (
            <button 
              key={tab}
        ...
        <div className="grid lg:grid-cols-2 gap-8 pb-20 animate-in fade-in duration-500">
          <div className="bg-white dark:bg-slate-800 rounded-[3rem] p-10 border border-slate-100 dark:border-slate-700 shadow-2xl h-fit relative overflow-hidden">
            ...
            <form onSubmit={(e) => { e.preventDefault(); setShowConfirmModal(true); }} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <input type="text" required disabled={isUploading} placeholder="e.g. Standard 8 Mathematics Unit 5" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full p-5 rounded-2xl border border-slate-100 dark:border-slate-700 outline-none focus:ring-2 focus:ring-emerald-500 bg-slate-50 dark:bg-slate-900 font-bold text-slate-800 dark:text-white" />
              </div>
              ...
              {activeAdminTab === 'content' ? (
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    {localFile && <p className="text-[10px] text-emerald-600 font-black uppercase tracking-widest mt-2">File Ready ({(localFile.size / 1024 / 1024).toFixed(2)} MB)</p>}
                  </div>
                </div>
              )}
          ...
          <div className="bg-white dark:bg-slate-800 rounded-[3rem] p-10 border border-slate-100 dark:border-slate-700 shadow-xl overflow-hidden flex flex-col h-fit">
            <h2 className="text-2xl font-black mb-8 text-slate-900 dark:text-white flex items-center gap-3">
               ...
               Asset Registry ({materials.length})
            </h2>
        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
          <div className="bg-emerald-800 p-10 md:p-16 rounded-[3rem] text-white flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl relative overflow-hidden">
             <div className="relative z-10 space-y-4 text-center md:text-left">
                ...
                <button onClick={() => onNavigate('admin-exam-form')} className="bg-white text-emerald-800 font-black px-10 py-4 rounded-2xl shadow-xl hover:scale-105 transition-all uppercase tracking-widest text-[11px]">Formulate Online Exam</button>
             </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-[3rem] p-10 border border-slate-100 dark:border-slate-700 shadow-2xl h-fit">
            ...
            <form onSubmit={handlePostAnnouncement} className="space-y-6">
              <div className="space-y-2">
                ...
                <input type="text" required value={annTitle} onChange={(e) => setAnnTitle(e.target.value)} placeholder="e.g. MSCE Results Update" className="w-full p-4 rounded-2xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 font-bold outline-none focus:ring-2 focus:ring-orange-500 transition-all" />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Message Content</label>
                <textarea required value={annContent} onChange={(e) => setAnnContent(e.target.value)} placeholder="Tell students what's new..." className="w-full p-4 rounded-2xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 font-medium outline-none focus:ring-2 focus:ring-orange-500 h-40 resize-none transition-all" />
              </div>
      ...
      {activeAdminTab === 'users' && (
        <div className="bg-white dark:bg-slate-800 rounded-[3rem] p-10 border border-slate-100 dark:border-slate-700 shadow-2xl animate-in fade-in duration-500 pb-20">
          <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight mb-8">Hub Members</h2>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {users.map(u => (
              <div key={u.id} className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-[2.5rem] border border-transparent hover:border-emerald-500/20 transition-all">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-black text-slate-900 dark:text-white">{u.name || 'User'}</h4>
